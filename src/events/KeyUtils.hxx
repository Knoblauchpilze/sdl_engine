  ...
  namespace core {
    namespace engine {
      ...
      namespace keyboard {
        ...
        Mode
        guessLayout() {
          // In order to determine the keyboard layout we will use the keycode
          // generated by carefully chosen scan code: this allows to determine
          // the association that is made between the physical scan codes and
          // the logical key codes and thus retrieve information about the type
          // of keyboard which produced this event.
          // More details can be found at `https://gist.github.com/g2p/8597984`.
          const SDL_Keycode qKC = SDL_GetKeyFromScancode(SDL_SCANCODE_Q);
          const SDL_Keycode wKC = SDL_GetKeyFromScancode(SDL_SCANCODE_W);
          const SDL_Keycode yKC = SDL_GetKeyFromScancode(SDL_SCANCODE_Y);

          // Cycle through main layouts.
          if (qKC == SDLK_q && wKC == SDLK_w && yKC == SDLK_y) {
            return Mode::Qwerty;
          }
          else if (qKC == SDLK_a && wKC == SDLK_z && yKC == SDLK_y) {
            return Mode::Azerty;
          }

          // No valid layout found: this is a problem.
          throw utils::CoreException(
            std::string("Could not guess keyboard layout"),
            std::string("guessLayout"),
            std::string("key_utils"),
            std::string("Q is mapped to ") + std::to_string(qKC) +
            ", W is mapped to " + std::to_string(wKC) +
            ", Y is mapped to " + std::to_string(yKC)
          );
        }
